...
// Interface removed for CommonJS compatibility

/**
 * Validates a CAIP identifier for correctness
 */
function validateCaip(balance) {
  const issues = [];
  let suggestedCaip;
  let severity = 'info';
 ...
 */
function detectDuplicateNativeCAIPs(balances) {
 ...
 */
function validateAndCorrectBalances(balances) {
  const corrected = [];
  const issues = [];
...
module.exports = {
  validateCaip,
  correctCaip,
  detectDuplicateNativeCAIPs,
  validateAndCorrectBalances
};