  ...
  try {
          ...
          if (portfolio.tokens && portfolio.tokens.length > 0) {
            ...
            for (const token of portfolio.tokens) {
              if (token.assetCaip && token.networkId) {
                ...
                const tokenAssetInfo =
                  assetData[token.assetCaip] || assetData[token.assetCaip.toLowerCase()];
                ...
                if (blockchains.includes(balanceString.networkId)) {
                  ...
                  const exists = balances.some(
                    (b: any) => b.caip === balanceString.caip && b.pubkey === balanceString.pubkey,
                  );
                  if (!exists) {
    ...
    try {
      ...
      if (mayaPubkey && mayaPubkey.address) {
        ...
        const hasMayaToken = balances.some(
            ...
            if (mayaBalanceResponse?.data && mayaBalanceResponse.data.length > 0) {
              ...
              for (const mayaBalance of mayaBalanceResponse.data) {
                if (mayaBalance.caip === 'cosmos:mayachain-mainnet-v1/denom:maya') {
                  ...
                  const mayaAssetInfo =
                    assetData[mayaBalance.caip] || assetData[mayaBalance.caip.toLowerCase()];
            ...
            for (const networkId of cosmosNetworks) {
              ...
              if (blockchains.includes(networkId)) {
                try {
                  console.log(
                    tag,
                    `Fetching staking positions for ${cosmosPubkey.address} on ${networkId}...`,
                  );
                  ...
                  if (stakingResponse?.data && Array.isArray(stakingResponse.data)) {
                    ...
                    for (const position of stakingResponse.data) {
                      ...
                      if (position.balance && position.balance > 0 && position.caip) {
                        ...
                        const stakingAssetInfo =
                          assetData[position.caip] || assetData[position.caip.toLowerCase()];
                        ...
                        const stakingBalance = {
                          ...
                          contextType: context.split(':')[0],
                          name:
                            stakingAssetInfo?.name || position.name || `${position.type} Position`,
                          caip: position.caip,
                          ...
                          valueUsd:
                            position.valueUsd || position.balance * (position.priceUsd || 0),
                    ...
                    console.log(
                      tag,
                      `No staking positions found for ${cosmosPubkey.address} on ${networkId}`,
                    );
                  }
                } catch (stakingError) {
                  console.error(
                    tag,
                    `Error fetching staking positions for ${cosmosPubkey.address} on ${networkId}:`,
                    stakingError,
                  );
                }
              }
